export async function exportGrid({ tiles, columns, showColor, mode }) {
  ...
  for (let i=0;i<tiles.length;i++){
    ...
    const x = col*(tileSize+spacing), y = row*(tileSize+spacing)
...
function drawSwatches(ctx, colors, x, y, w, h) {
  ...
  for (let i=0;i<colors.length;i++){
    ...
    const cx = x + pad + i*(circle+gap), cy = y + h - pad - circle
